...
const schema = z.object({
  ...
  raza: z.string().min(2, 'Mínimo 2 caracteres').max(100),
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-4 bg-white border border-gray-200 rounded space-y-3" aria-labelledby="form-title">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
          <label className="block text-sm mb-1" htmlFor="nombre">Nombre <span className="text-red-600">*</span> <Tooltip label="Campo obligatorio. Mínimo 2 caracteres." /></label>
        ...
        <div>
          <label className="block text-sm mb-1" htmlFor="raza">Raza <span className="text-red-600">*</span> <Tooltip label="Campo obligatorio. Mínimo 2 caracteres." /></label>
